...
export async function GET(req: Request) {
    ...
    const bibtex = await promises.readFile('public/citations/bibtex.bib', 'utf-8');
    
    const refs = bibtex.split(/(?=@\w+{)/).filter(entry => entry.trim() !== "");
    const refsObj = refs.reduce((acc, ref) => {
        ...
        const contents = ref.replace(/^@\w+{[^,]+,/, "").trim().split("\n").filter(line => line.trim() !== "");
        ...
        acc[bibref] = { type, bibref, authorsList: [], ...entries.reduce((entryAcc, entry) => ({ ...entryAcc, [entry.key]: entry.value }), {}) };
        return acc;
    }, {});

    if (isToSave) {
        promises.writeFile('public/citations/bibtex.json', JSON.stringify(refsObj, null, 2)).catch(console.error);
    }
