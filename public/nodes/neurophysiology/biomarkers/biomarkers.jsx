...
const Biomarkers = new MyNode("Biomarkers", [], (<div>
      <ul>
        <li>Excitatory:</li>
        {biomarkers.excitatory.map((excitatory, index) => (<li key={`excitatory-${index}`}>{excitatory}</li>))}
      </ul>
      <ul>
        <li>Inhibitory:</li>
        {biomarkers.inhibitory.map((inhibitory, index) => (<li key={`inhibitory-${index}`}>{inhibitory}</li>))}
      </ul>
      ...
      <CitationCard citations={[InferringEIFromFieldPotentials]}>
        ...
        2004).
        <Figure src="images/ei/ei_frequency.png" alt="E:I interaction model" caption="Model schematic: an “LFP population” receives input from two Poisson populations, one excitatory and one inhibitory." n={1}/>
      </CitationCard>
